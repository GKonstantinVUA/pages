(() => {
    ...
    window.onscroll = () => {
        ...
        header.classList.toggle("sticky", window.scrollY > 98);
    };
    ...
    window.addEventListener("load", window.onRefreshLinisScroll);
    document.addEventListener("slideDownDone", window.onRefreshLinisScroll);
    document.addEventListener("slideUpDone", window.onRefreshLinisScroll);
    window.addEventListener("hashchange", (() => {
        const currentHash = location.hash || "#home";
        const menuLinks = document.querySelectorAll(".menu__link, .spollers__menu-link");
        menuLinks.forEach((menuLink => {
            menuLink.classList.remove("_active-page", "_active-footer");
        }));
        menuLinks.forEach((menuLink => {
            if (menuLink.getAttribute("href") === currentHash) if (menuLink.classList.contains("menu__link")) menuLink.classList.add("_active-page"); else if (menuLink.classList.contains("spollers__menu-link")) menuLink.classList.add("_active-footer");
        }));
    }));
    window["FLS"] = false;
    menuInit();
